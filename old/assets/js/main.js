  ...
  var onMembers = function(githubData, status, xhr) {
    ...
    for (var i = 0; i < members.length; i++) {
      membersRendered += MEMBER.replace('#{url}', members[i].avatar_url)
        ...
        .replace(
          '#{url}',
          members[i].url.replace('api.', '').replace('users/', '')
        )
    }
    $('.members')
      .empty()
      .append(membersRendered)
    $('.member').fadeIn(1000)
  }

  ajaxWithURI(
    'https://api.github.com/orgs/globocom/public_members?page=1&per_page=100'
  ).always(onMembers)
})(jQuery)
